...
struct RProc* mrb_ext_parse_nstring_as_proc(mrb_state *mrb, const char *s, size_t len)
{
  struct mrb_parser_state* p = mrb_parse_nstring(mrb, s, len, NULL);
  struct RProc* ret = mrb_generate_code(mrb, p);
  return ret;
}

const mrb_irep * mrb_ext_irep_from_rproc(struct RProc *proc)
{
  return proc->body.irep;
}

const char * mrb_ext_str_from_pool(const mrb_irep *irep, size_t n)
{
  if ((irep->pool[n].tt & IREP_TT_NFLAG) == 0) {
    return irep->pool[n].u.str;
  }
  else {
    return "";
  }
...
const mrb_irep * mrb_ext_subirep_from_rproc(struct RProc *proc, size_t n)
{
  return proc->body.irep->reps[n];
}
